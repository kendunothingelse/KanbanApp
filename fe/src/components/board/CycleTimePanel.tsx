    ...
    return (
        <Card sx={{ p: 2.5, border: `1px solid ${palette.border.light}` }}>
            ...
            <Typography variant="body2" color="text.secondary" mb={2}>
                Mất bao lâu để hoàn thành từng công việc. Đường đỏ = trung bình.
            </Typography>
            <Box height={280}>
                {data.length ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 50 }}>
                            ...
                            <XAxis dataKey="name" interval={0} angle={-35} textAnchor="end" height={50} tick={{ fontSize: 10, fill: palette.text.secondary }} />
                            <YAxis label={{ value: "Ngày", angle: -90, position: "insideLeft", fontSize: 11 }} tick={{ fontSize: 11, fill: palette.text.secondary }} />
                            ...
                            <ReferenceLine y={avgCycle} stroke={palette.accent.main} strokeDasharray="4 4" strokeWidth={2} />
                        </BarChart>
                    </ResponsiveContainer>
                ) : (
